...
sleep(2);
   ...
   cout << "<Captain#2> is setting up" << endl;
   sleep(2);
...
status = getaddrinfo("nunki.usc.edu", "21430", &hints, &res); //21300 + 130
  ...
  if(status != 0){
    ...
    //TODO finish display
    cout << "<Captain#2 has TCP port ";


    cout << "21430";   //local_port?
...
cout << endl;
   ...
   bytes_recv = recv(new_fd, buf, 5, 0);
    ...
    cout << "<Captain#2> is now connected to General" << endl;
    
    if(!buf[0]){
	cout << "<Captain#2 has started the Mission!!!!!!" << endl;
    }
    else if(buf[0]){
	cout << "<Captain#2 is back up for the mission!" << endl;
    }

    cout << "End of Phase 3 for Captain#2" << endl;
